...
import { Laptop, Mouse, Keyboard, Monitor, Plus, Package, HardDrive, User } from 'lucide-react';
...
const AssetsLanding = () => {
  ...
  const handleAddAssetType = async (e) => {
    e.preventDefault();
    if (!window.confirm(`Are you sure you want to add "${newTypeName}" as a new asset type?`)) return;
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
        {assetTypes.map((type) => (
          ...
          >
            ...
            <p className="text-gray-400 text-sm mt-2">View all records</p>
          </div>
        ))}
      </div>
